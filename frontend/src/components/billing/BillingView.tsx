...
export const BillingView: React.FC = () => {
  ...
  const pagedSubscriptions = (subscriptions as any[]).slice((subPage - 1) * PAGE_SIZE, subPage * PAGE_SIZE);
  const pagedInvoices = (invoices as any[]).slice((invPage - 1) * PAGE_SIZE, invPage * PAGE_SIZE);
            ...
            <tbody className="divide-y divide-slate-100">
              {pagedSubscriptions.length === 0 ? (
                ...
                pagedSubscriptions.map((sub: any) => {
                  const prodName = sub.product_name || sub.productName || 'Subscription';
                  const orderId = sub.order_id || sub.orderId || '';
                  const interval = sub.billing_interval || sub.interval || 'MONTHLY';
                  const unitPrice = Number(sub.unit_price ?? sub.amount ?? 0);
                  const startDate = sub.start_date ? new Date(sub.start_date).toLocaleDateString() : (sub.startDate || '-');
                  const nextBillDate = sub.next_billing_date ? new Date(sub.next_billing_date).toLocaleDateString() : (sub.nextBillingDate || '-');
                  return (
                    <tr key={sub.id} className="hover:bg-slate-50 transition">
                      <td className="py-3 px-4">
                        <div className="font-semibold text-slate-900">{prodName}</div>
                        <div className="text-[10px] text-slate-400 font-mono">Order: {orderId.toUpperCase()}</div>
                      </td>
                      ...
                      <td className="py-3 px-3 font-mono font-bold text-slate-900">
                        ${unitPrice.toLocaleString()}/mo
                      </td>
                      <td className="py-3 px-3 font-mono text-slate-600">{startDate}</td>
                      <td className="py-3 px-3 font-mono text-slate-800 font-semibold">{nextBillDate}</td>
                      <td className="py-3 px-3">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            onClick={() => modifySubMutation.mutate({ subId: sub.id, newQty: Number(sub.quantity) + 1 })}
            ...
            <tbody className="divide-y divide-slate-100">
              {pagedInvoices.length === 0 ? (
                ...
                pagedInvoices.map((inv: any) => {
                  ...
                  const invType = inv.invoice_type || inv.type || 'ONE_TIME';
                  ...
                  const dueDate = inv.due_date ? new Date(inv.due_date).toLocaleDateString() : (inv.dueDate || '-');
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="py-3.5 px-3">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                          invType === 'ONE_TIME' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-purple-50 text-purple-700 border border-purple-200'
                        }`}>
                          {String(invType).replace('_', ' ')}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-3 font-mono text-slate-500">{dueDate}</td>
                      <td className="py-3.5 px-3">
