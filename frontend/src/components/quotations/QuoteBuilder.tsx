...
export const QuoteBuilder: React.FC = () => {
  ...
  const riskFactors: string[] = useMemo(() => {
    if ((riskData as any)?.line_risks && (riskData as any).line_risks.length > 0) {
      const violations = (riskData as any).line_risks
                    ...
                    return (
                      <tr key={line.id} className={`hover:bg-gray-50 transition ${hasViolation ? 'bg-red-50/20' : ''}`}>
                        <td className="py-4 px-5">
                          ...
                          {hasViolation && (
                            <div className="flex items-center gap-1 text-[10px] text-red-600 font-medium mt-1">
                              ...
                              <span>Discount ({discountPercentNum}%) exceeds ceiling ({Number((line as any).allowed_discount_percent ?? (line as any).allowed_discount_ceiling ?? 0)}%)</span>
                            </div>
                          )}
                        </td>
                        ...
                        <td className="py-4 px-3">
                          ...
                          {((line as any).allowed_discount_percent !== undefined || (line as any).allowed_discount_ceiling !== undefined) && (
                            <span className="text-[10px] text-gray-400 block mt-0.5 font-mono">
                              Limit: {Number((line as any).allowed_discount_percent ?? (line as any).allowed_discount_ceiling)}%
                            </span>
                          )}
                        </td>
            ...
            {riskData?.approval_required ? (
              <div className="flex items-center gap-2 mt-2">
                <span className="px-2.5 py-1 rounded bg-blue-600 text-white font-bold text-xs shadow-xs">
                  {(riskData as any)?.required_approval_level || (riskData as any)?.approval_level || (quotation as any).current_approval_level || (quotation as any).approval_level || 'Required'}
                </span>
              </div>
            ) : (
