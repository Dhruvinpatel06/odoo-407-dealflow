      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-xs overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="text-sm divide-y divide-gray-100">
                {(role === 'SALES_REP' ? (repQuotes.length > 0 ? repQuotes : quotations) : quotations)
                  .slice((quotePage - 1) * PAGE_SIZE, quotePage * PAGE_SIZE)
                  .map((q: any) => {
                  const marginNum = Number(q.margin_percent ?? q.blendedMarginPercent ?? 0);
                  const totalNum = Number(q.total_amount ?? q.totalAmount ?? 0);
                  const riskNum = Number(q.risk_score ?? q.blendedRiskScore ?? 0);
                  const qNum = q.quotation_number ?? q.quoteNumber ?? 'Quote';
                  const cName = q.customer_name ?? q.customerName ?? 'Account';
                  const tierName = q.customer_tier_name ?? q.customerTier;
                  const repName = q.sales_rep_name ?? q.salesRepName ?? 'Sales Rep';
                  const statusVal = q.status ?? q.stage ?? 'DRAFT';
                  const riskStatus = q.risk_status ?? q.riskStatus;
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-4 font-mono text-xs text-gray-400">{qNum}</td>
                      <td className="px-5 py-4 font-bold text-gray-900">
                        <div>{cName}</div>
                        {tierName && (
                          <div className="text-[10px] text-gray-400 font-normal">Tier: {tierName}</div>
                        )}
                      </td>
                      {role !== 'SALES_REP' && (
                        <td className="px-5 py-4 text-xs text-gray-600">{repName}</td>
                      )}
                      ...
                      <td className="px-5 py-4 text-center">
                        <RiskBadge score={riskNum} status={riskStatus} />
                      </td>
          ...
          <PaginationControls
            ...
            totalItems={(role === 'SALES_REP' ? (repQuotes.length > 0 ? repQuotes : quotations) : quotations).length}
            pageSize={PAGE_SIZE}
            ...
            {pendingApprovals
              ...
              >
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="font-bold text-gray-900">{app.quotation_number || app.quoteNumber || 'Workflow'} — {app.customer_name || app.customerName || 'Account'}</span>
                  <span className="font-mono font-bold text-gray-900">${Number(app.total_amount || app.amount || 0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] mt-1.5">
                  <span className="text-gray-600 font-medium">Rep: {app.sales_rep_name || app.salesRepName || 'Assigned Rep'}</span>
                  <span className="px-2 py-0.5 rounded bg-white text-gray-800 border border-gray-200 font-semibold text-[10px]">
                    {role === 'SALES_REP' ? 'Awaiting Review' : 'Requires Sign-Off'}
                  </span>
                </div>
              </div>
            ))}
            ...
            {auditLogs.slice(0, 3).map((log: any) => (
              <div key={log.id} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-100 text-xs">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-gray-900 truncate">{log.userName || log.user_name} ({log.userRole || log.user_role})</span>
                    <span className="text-[10px] text-gray-400 font-mono shrink-0">
                      {new Date(log.timestamp || log.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  <div className="text-gray-700 font-medium mt-0.5">{log.action || log.event_type}</div>
                  {(log.reason || log.comment) && <p className="text-gray-500 text-[11px] mt-0.5">{log.reason || log.comment}</p>}
                </div>
              </div>
            ))}
          </div>
