        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {products.slice((catalogPage - 1) * PAGE_SIZE, catalogPage * PAGE_SIZE).map((p: any) => {
                const price = Number(p.unit_price ?? p.unitPrice ?? 0);
                ...
                const discountLimit = Number(p.max_discount_percent ?? p.max_discount_ceiling ?? p.discountLimit ?? 15);
                return (
                  <tr key={p.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="py-3 px-3 font-mono font-bold text-slate-900">${price.toLocaleString()}</td>
                    <td className="py-3 px-3 font-mono text-slate-600">${cost.toLocaleString()}</td>
      ...
      {activeTab === 'SUBSCRIPTIONS' && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {subscriptions.slice((subPage - 1) * PAGE_SIZE, subPage * PAGE_SIZE).map((s: any) => (
                <tr key={s.id}>
